...
public class MainGui : MonoBehaviour
{
    ...
    private string mShowText = "token";   

    void Start()
    {
        // Select the Google Play Games platform as our social platform implementation
        PlayGamesPlatform.Activate();
        PlayGamesClientConfiguration googleClientConfig = new PlayGamesClientConfiguration.Builder()
            .EnableSavedGames()
            .RequestEmail()
            ...
            if (!Social.localUser.authenticated)
            {
                ...
                Social.localUser.Authenticate((bool success) =>
                {
                    ...
                    if (success)
                    {
                        ...
                        string token = "111";
                        if (((PlayGamesLocalUser)Social.localUser).GetIdToken() == null)
                            token = "null";
                        showtext.text = string.Format("token: {0}", token);
                    }
                    else {
